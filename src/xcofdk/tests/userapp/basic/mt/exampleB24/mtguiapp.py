...
from xcofdk.tests.userapp.basic.mt.exampleB24.maintask import MainTaskNoSerDes
...
def Main(fwStartOptions_ : list):
    ...
    _myMXT = MainTaskNoSerDes()
    if _myMXT.isDetachedFromFW:
        xlogif.LogError('[exampleB24] Failed to create main task.')
        return 71
...
if __name__ == "__main__":
    for aa in sys.argv:
        if aa == '--help':
            ...
            _usage  = f'Usage:\n\t$> python3 -m {_usage} [--help] [--enable-async-execution] [--service-tasks-count NUM] [--disable-auto-close] [--disable-auto-start] [--disable-log-timestamp] [--disable-log-highlighting] [--enable-log-callstack] [--log-level LLEVEL]'
