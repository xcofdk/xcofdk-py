...
from xcofdk.tests.userapp.basic.mt.exampleB31.maintask import MainTaskGIL
...
def Main(fwStartOptions_ : list):
    ...
    _myMXT = MainTaskGIL.CreateSingleton(None)
    if _myMXT is None:
        return 71
...
if __name__ == "__main__":
    for aa in sys.argv:
        if aa == '--help':
            ...
            _usage  = f'Usage:\n\t$> python3 -m {_usage} [--help] [--force-deficient-frequency] [--fibonacci-input FIBO_NUM] [--disable-auto-close] [--disable-auto-start] [--disable-log-timestamp] [--disable-log-highlighting] [--enable-log-callstack] [--log-level LLEVEL]'
