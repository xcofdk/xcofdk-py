...
from xuserapp.basic.xmt.rc.rcmisc.servicetask    import CreateServiceTask
from xuserapp.basic.xmt.rc.rcmisc.servicetaskBXQ import CreateServiceTaskBXQ


# ------------------------------------------------------------------------------
# Interface
# ------------------------------------------------------------------------------
def _CreateMainTask( cmdLineOpts_        : CLOptions
                   , guiTitle_           : str =None
                   , bBroadcast_               =False
    ...
    _phasedXFCallback = XFMainTask( cmdLineOpts_
                                  , guiTitle_=guiTitle_
                                  , bBroadcast_=bBroadcast_
                                  , bBlockingSrvTask_=bBlockingSrvTask_
                                  , bSkipPayloadSerDes_=bSkipPayloadSerDes_
                                  , bCustomPayLoad_=bCustomPayLoad_)

    if cmdLineOpts_.isAsynMainTaskEnabled:
        res = XFAsyncCommTask(_phasedXFCallback, aliasName_=_aliasName, bMainTask_=True)
    else:
        res = XFSyncCommTask(_phasedXFCallback, aliasName_=_aliasName, bMainTask_=True)
    return res
#END _CreateMainTask()
...
class XFMainTask(UserAppControllerIF):
    ...
    __slots__ = [ '__gui' , '__mdl' , '__cmdLine' , '__ctrSnd' , '__ctrRcv' , '__lstSrv' , '__lstSrvNR'
                , '__dictSrvInfo' , '__curSrvView' , '__startTime' , '__cntErrSnd' , '__bSkipSerDes'
                , '__bPostingPaused' , '__bBlockingSrvTsk' , '__ctrOutOfOrder' , '__dictLastRcvUID'
                , '__guiTitle' , '__bCustomPLD' , '__taskInfo' , '__bBroadcast' , '__myCTsk' ]
    ...
    def OnViewEncounteredException(self, errMsg_ : str, xcp_ : Exception):
        ...
        if self.__myTask.isFatalErrorFree:
            ...
            xlogif.LogExceptionEC(errMsg_, xcp_, 2021001)
    # --------------------------------------------------------------------------
    ...
    def __CreateStartServiceTasks(self) -> int:
        ...
        for ii in range(self.__cmdLine.serviceTasksCount):
            ...
            if self.__bBlockingSrvTsk:
                _srv = CreateServiceTaskBXQ(ii+1)
            else:
                _srv = CreateServiceTask(ii+1, bBroadcast_=self.__bBroadcast, bSkipPayloadSerDes_=self.__bSkipSerDes, bCustomPayLoad_=self.__bCustomPLD)
    ...
    def __PostMessage(self, srvTask_ : IRCCommTask =None, msgLabelID_ : EMsgLabelID =None, msgClusterID_ : EMsgClusterID =None) -> bool:
        ...
        if self.__bBroadcast:
            ...
            # broadcast message
            _uid = self.__myTask.BroadcastMessage(msgLabelID_)
        ...
        for ii in range(len(_lst)):
            ...
            # send message
            _uid = self.__myTask.SendMessage(_srv, msgLabelID_=msgLabelID_, msgClusterID_=msgClusterID_)
