...
class IXTask(ITask):
    """
    Subclassing the parent class ITask this interface class is inheritd by
    the abstract class XTask whose instances represent application tasks
    created by the framework and provided to applications.

    THe major part of this intereface is described by its parent class:
        >>> ITask
    ...
    Deprecated API:
    ----------------
    Starting with XCOFDK-py v3.0 below API functions/properties (formerly part
    of class XTask) are deprecated and not available anymore:
        >>> @property
        >>> def xtaskProfile(self) -> ITaskProfile:
    ...
    def TriggerExternalQueueProcessing(self) -> int:
        ...
        ------
            ...
            - The request will be ignored by the framework (returnning 0) if:
                  - messaging subsystem 'xmsg' is disabled via RTE policy
                    configuration,
                  - this instance doesn't have an external queue,
                  - this instance is not the currently running task,
    ...
    def isMainTask(self) -> bool:
        """
        Getter property to find out whether this instnace is the main
        application task instance.

        Returns:
        ----------
            True, if this instance is the singleton of the class RCTask or of
            a (user-defined) class subclassing class XMainTask, respectively,
            False otherwise.
    ...
    def taskProfile(self) -> ITaskProfile:
        ...
        Returns:
        ----------
            If successfully started and as long as attached to framework,
            reference to a read-only copy of the task profile argument passed
            to the constructor of class XTask (or XMainTask) when this instance
            was created, reference to that argument itself otherwise.
