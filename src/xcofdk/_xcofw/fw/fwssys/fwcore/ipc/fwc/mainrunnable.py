...
class _MainRunnable(_AbstractRunnableFWC):
    ...
    def _RunExecutable(self):
        ...
        if self.__lcMonImpl.isLcMonitorIdle:
            self.__lcMonImpl._ActivateLcMonitorAlivenessByMR(self.executionProfile.runPhaseFreqMS)
    ...
    def _OnRunProgressNotification(self, eRunProgressID_ : _ERunProgressID):
        ...
        if eRunProgressID_.isReadyToRun:
            ...
            while not self._lcProxy.isLcModeNormal:
    ...
    def __StartFWCs(self):
        ...
        for _kk in _keys:
            ...
            res = _vv.Start()
            if not res:
                vlogif._LogOEC(True, -1497)
                break

            res = _vv.isRunning
            if not res:
                vlogif._LogOEC(True, -1498)
                break
        ...
        if res:
            if not _FwSubsystemCoding.IsSubsystemMessagingConfigured():
                ...
                if _rblDisp is None:
                    ...
                    res = self._RegisterDefaultDispatchFilter()
                    if res:
                        for _kk in _keys:
                            _fwcID = _EFwcID(_kk)
                            ...
                            res = _vv._RegisterDefaultDispatchFilter()
                            if not res:
                                break
    ...
    def __AddFwc(self, eFwcID_ :  _EFwcID, fwc_ : _AbstractRunnableFWC):
        if not (isinstance(eFwcID_,  _EFwcID) and isinstance(fwc_, _AbstractRunnableFWC)):
            vlogif._LogOEC(True, -1499)
            return False
        elif eFwcID_.value in self.__fwcDict:
            vlogif._LogOEC(True, -1500)
            return False
    ...
    def __NotifyLcFailure(self, eFailedCompID_ : _ELcCompID, frcError_ : _FatalEntry, atask_ : _AbstractTask):
        self._lcProxy._NotifyLcFailure(eFailedCompID_, frcError_, atask_=atask_)
    ...
    def __CheckProcessingFeasiblity(self, eCallbackID_ : _EErrorHandlerCallbackID =None, curFatalError_ : _FatalEntry =None) -> _ETernaryOpResult:
        ...
        if _bFE and not _bEHCB:
            ...
            _frc = logif._CreateLogImplError(_wmsg)
        ...
        if _bFE:
            _errPart = _FwTDbEngine.GetText(_EFwTextID.eMisc_Shared_FmtStr_023).format(curFatalError_.uniqueID, curFatalError_.shortMessage)
            _tailPart = _errPart
        else:
            _errPart = None
        ...
        _bHasLcAnyFailureState = self._lcProxy.hasLcAnyFailureState

        if _bHasLcAnyFailureState:

            _frcv = self._lcProxy.lcFrcView
            ...
            if _lstCandid is not None:
                for _myFE in _lstCandid:
                    if _myFE.isInvalid:
                        ...
                        at = _myFE._taskInstance
                        if (at is None) or not at.isValid:
                            at = self._drivingTask
                        self.__NotifyLcFailure(_ELcCompID.eFwMain, _myFE, at)
                    _myFE._ForceCleanUp()
                _lstCandid.clear()
        return _ETernaryOpResult.MapExecutionState2TernaryOpResult(self)
    ...
    def __CheckStoredFFEsOnLcFailureCandidacy(self) -> list:
        ...
        for _ii in range(len(_lstPFFE)):
            ...
            _bTaskMissedToResolveFE = _ti.eTaskExecPhase != _myFE.eTaskExecPhase  
