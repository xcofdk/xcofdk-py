...
class _StopWatch(_AbstractSlotsObject):
    ...
    def __init__(self, usTimeTicksStart_ : int =None, usTimeTicksStop_ : int=None):
        ...
        self.__start = usTimeTicksStart_
    ...
    def _ToString(self, *args_, **kwargs_):
        ...
        res = _FwTDbEngine.GetText(_EFwTextID.eStopWatch_ToString_01).format(self.__start)
        if self.__stop is not None:
            res += _FwTDbEngine.GetText(_EFwTextID.eStopWatch_ToString_02).format(self.__stop, self.__td)
        return res
    ...
    def __Stop(self, usTimeTicksStop_ : int, bRelative_ : bool) -> _TimeDelta:
        ...
        if bRelative_:
            _usTimeTicksRef = self.__start if self.__stop is None else self.__stop
        else:
...
class _KpiLogBook(_AbstractSlotsObject):
    ...
    @property
    def eStartKPI(self):
        return None if self.__isInvalid else self.__startID
